...
import (
	...
	client "github.com/threeport/wordpress-threeport-extension/pkg/client/v0"
)
...
func v0WordpressInstanceDeleted(
	...
	// delete gateway instance if it exists
	gatewayInstanceIds, err := tpclient.GetObjectIdsByAttachedObject(
		r.APIClient,
		r.APIServer,
		tpapi.ObjectTypeGatewayInstance,
		v0.ObjectTypeWordpressInstance,
	...
	if err != nil {
		return 0, fmt.Errorf("failed to get attached gateway instance IDs: %w", err)
	}
	for _, gatewayInstanceId := range gatewayInstanceIds {
		...
		if err != nil {
			return 0, fmt.Errorf("failed to delete gateway instance with ID %d: %w", *gatewayInstanceId, err)
		}
		if err := tpclient.EnsureAttachedObjectReferenceRemoved(
			r.APIClient,
			r.APIServer,
			...
			return 0, fmt.Errorf("failed to remove attachment to deleted gateway instance: %w", err)
		}
	}

	// delete domain name instance if it exists
	domainNameInstanceIds, err := tpclient.GetObjectIdsByAttachedObject(
		r.APIClient,
		r.APIServer,
		tpapi.ObjectTypeDomainNameInstance,
		v0.ObjectTypeWordpressInstance,
	...
	if err != nil {
		return 0, fmt.Errorf("failed to get attached domain name instance IDs: %w", err)
	}
	for _, domainNameInstanceId := range domainNameInstanceIds {
		_, err := tpclient.DeleteGatewayInstance(
			r.APIClient,
			r.APIServer,
		...
		if err != nil {
			return 0, fmt.Errorf("failed to delete domain name instance with ID %d: %w", *domainNameInstanceId, err)
		}
		if err := tpclient.EnsureAttachedObjectReferenceRemoved(
			r.APIClient,
			r.APIServer,
			...
			return 0, fmt.Errorf("failed to remove attachment to deleted domain name instance: %w", err)
		}
	}
