...
import (
	...
	wordpress "github.com/threeport/wordpress-threeport-extension/internal/wordpress"
	notif "github.com/threeport/wordpress-threeport-extension/internal/wordpress/notif"
	zap "go.uber.org/zap"
	"net/http"
	"os"
	"sync"
...
func main() {
	// flags
	var wordpressDefinitionConcurrentReconciles = flag.Int(
		"wordpress-definition-concurrent-reconciles",
		1,
		"Number of concurrent reconcilers to run for wordpress definitions",
	)
	var wordpressInstanceConcurrentReconciles = flag.Int(
		"wordpress-instance-concurrent-reconciles",
		1,
		"Number of concurrent reconcilers to run for wordpress instances",
	)
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      wordpress.LockBucketName,
		Description: wordpress.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", wordpress.LockBucketName)
		os.Exit(1)
	}

	// check to ensure wordpress stream has been created by API
	wordpressStreamNameFound := false
	for stream := range js.StreamNames() {
		if stream == notif.WordpressStreamName {
			wordpressStreamNameFound = true
		}
	}
	if !wordpressStreamNameFound {
		log.Error(errors.New("JetStream stream not found"), "failed to find stream with wordpress stream name", "wordpressStreamName", notif.WordpressStreamName)
		os.Exit(1)
	}
	...
	reconcilerConfigs = append(reconcilerConfigs, controller.ReconcilerConfig{
		ConcurrentReconciles: *wordpressDefinitionConcurrentReconciles,
		Name:                 "WordpressDefinitionReconciler",
		NotifSubject:         notif.WordpressDefinitionSubject,
		ReconcileFunc:        wordpress.WordpressDefinitionReconciler,
	})
	reconcilerConfigs = append(reconcilerConfigs, controller.ReconcilerConfig{
		ConcurrentReconciles: *wordpressInstanceConcurrentReconciles,
		Name:                 "WordpressInstanceReconciler",
		NotifSubject:         notif.WordpressInstanceSubject,
		ReconcileFunc:        wordpress.WordpressInstanceReconciler,
	})
	...
	for _, r := range reconcilerConfigs {
		...
		js.AddConsumer(notif.WordpressStreamName, &natsgo.ConsumerConfig{
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(notif.WordpressStreamName))
		if err != nil {
		...
		reconciler := controller.Reconciler{
			...
			EventsRecorder: &event.EventRecorder{
				APIClient:           apiClient,
				APIServer:           *apiServer,
				ReportingController: "WordpressController",
			},
			JetStreamContext: js,
			KeyValue:         kv,
			Log:              &log,
	...
	log.Info(
		"wordpress controller started",
		"version", version.GetVersion(),
		"controllerID", controllerID.String(),
		"NATSConnection", natsConn,
		"lockBucketName", wordpress.LockBucketName,
	)
	...
	log.Info("wordpress controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-wordpress-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}
